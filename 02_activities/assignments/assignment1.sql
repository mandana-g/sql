...
SELECT product_id, product_name, 
	   ...
	   END AS prod_qty_type_condensed,
	  CASE 
	    	WHEN lower(product_name) like lower("%pepper%") THEN 1
	    	ELSE 0
	  END AS pepper_flag	
FROM product;
...
SELECT * FROM vendor
INNER JOIN vendor_booth_assignments on
...
SELECT vendor.vendor_id, count(vendor_booth_assignments.vendor_id) AS number_of_renting_booths
FROM vendor 
...
SELECT *, sum(customer_purchases.quantity * customer_purchases.cost_to_customer_per_qty) as all_amount_spent FROM customer 
LEFT JOIN customer_purchases ON customer.customer_id= customer_purchases.customer_id 
...
*/
CREATE TEMPORARY TABLE temp.newvendor
AS SELECT * FROM vendor;

INSERT INTO temp.newvendor 
...
and year are! */
SELECT customer_id, strftime('%Y', market_date) AS year, strftime('%m',market_date) AS month 
...
SELECT customer_id, sum(quantity*cost_to_customer_per_qty) as all_amount_spent, strftime('%Y', market_date) AS year, strftime('%m',market_date) AS month 
FROM customer_purchases
